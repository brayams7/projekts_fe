  ...
  endpoints: (builder) => (
    {
      getAllWorkspaces: builder.query({
        ...
        ), providesTags: [TYPES_WORKSPACE.TYPE_LIST_WORKSPACES], keepUnusedDataFor: 120
      ...
      getAllWorkspacesUser: builder.query({
        ...
        ), providesTags: (result) =>
        {
          ...
          return result && Array.isArray(list) ? [
            ...list.map(({ id }) => (
              { type: TYPES_WORKSPACE.TYPE_LIST_WORKSPACES_USER, id: id }
            )), {
          ...
          try
          {
            ...
            if (Array.isArray(list))
            {
              const data = list.map((item) =>
              {
                ...
                return {
                  ...item, label: item.name, value: item.id, created_at, updated_at
                };
              });
      ...
      createWorkspace: builder.mutation({
        query: (body) => (
          {
            url: "/workpaces", method: "POST", body: body
          }
      ...
      updateWorkspace: builder.mutation({
        ...
        {
          return {
            url: `/workpaces/${workspaceId}`, method: "PUT", body: body
          };
        }, invalidatesTags: (result, error, body) =>
        {
          return [{ type: TYPES_WORKSPACE.TYPE_LIST_WORKSPACES_USER, id: body.user_id }];
      ...
      inviteMemberToWorkspace: builder.mutation({
        ...
        {
          return {
            url: `/workspace/inviteMemberToWorkspace/${workspaceId}`, method: "POST", body
          };
        ...
        {
          return {
            url: `/workspace/acceptInvitationToWorkspace/`, method: "POST", body
          };
        }, invalidatesTags: [TYPES_WORKSPACE.TYPE_WORKSPACE_BY_ID]
      })
    }
